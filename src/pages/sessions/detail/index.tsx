...
function MotorGripChart({ metrics }: { metrics: Extract<GameMetrics, { mode: 'MOTOR_GRIP' }> }) {
  const reduceMotion = useReducedMotion();
  const values = [{ label: 'Puncak genggaman', value: metrics.peakGripPercent, display: `${Math.round(metrics.peakGripPercent)}%` }, { label: 'Durasi tahan', value: Math.min((metrics.continuousHoldMs / 5000) * 100, 100), display: `${(metrics.continuousHoldMs / 1000).toFixed(1)} dtk` }];
  return <section aria-labelledby="session-chart-title" className="mt-7 rounded-md border-2 border-divider bg-white p-5 sm:p-6"><p className="m-0 text-sm font-black tracking-[0.08em] text-muted uppercase">Visual performa</p><h2 className="mt-2 mb-0 text-2xl font-black" id="session-chart-title">Genggaman dan durasi tahan</h2><div className="mt-7 grid gap-7">{values.map((item) => <div key={item.label}><div className="flex items-center justify-between gap-4"><span className="font-black">{item.label}</span><strong className="text-xl">{item.display}</strong></div><div aria-hidden className="mt-3 h-6 overflow-hidden rounded-full bg-divider"><m.div animate={{ width: `${item.value}%` }} className="h-full rounded-full bg-gradient-to-r from-brand to-[#e7ad27]" initial={{ width: reduceMotion ? `${item.value}%` : 0 }} transition={{ duration: 0.75, ease: 'easeOut' }} /></div></div>)}</div></section>;
}
