...
function SetupPanel({ mode, snapshot, canStart, fruit }: { mode: GameMode; snapshot: SetupSnapshot | null; canStart: boolean; fruit: FruitVariant }) {
  ...
  if (mode === 'GO_NO_GO') return <div className="grid min-h-80 place-items-center bg-white p-8 text-center"><div><span className="mx-auto grid size-20 place-items-center rounded-full bg-[#eaf3ff]"><Hand aria-hidden className="size-10 text-[#3978bd]" /></span><h3 className="mt-5 mb-0 text-4xl font-black">{canStart ? 'Alat siap' : snapshot?.instruction ?? 'Genggam alat satu kali.'}</h3>{canStart && <p className="mt-3 mb-0 text-lg font-bold text-muted">Tekan Mulai saat sudah siap.</p>}</div></div>;
  return <div className="grid min-h-80 place-items-center p-8 text-center"><div><SqueezableFruit fruit={fruit} showLabel={false} squeezePercent={canStart ? 45 : 10} /><h3 className="mt-5 mb-0 text-4xl font-black">{canStart ? 'Alat siap' : snapshot?.instruction ?? 'Genggam alat satu kali.'}</h3>{canStart && <p className="mt-3 mb-0 text-lg font-bold text-muted">Tekan Mulai saat sudah siap.</p>}</div></div>;
}
