...
import { ArrowLeft, Camera, Clock3, ImageOff, Mars, Pencil, Venus } from 'lucide-react';
...
function OverallModePanel({ summary }: { summary: ParticipantDetailDto['modeSummaries'][number] | undefined }) {
  ...
  if (metrics.mode === 'MOTOR_GRIP') {
    ...
    return <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-center"><div><p className="m-0 text-sm font-black tracking-[0.08em] text-muted uppercase">Overall {summary.savedSessionsTotal} permainan</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="border-l-4 border-divider py-1 pl-4"><span className="text-sm font-bold text-muted">Rata-rata skor</span><strong className="mt-2 block text-3xl">{metrics.averageScore}</strong></div><div className="border-l-4 border-divider py-1 pl-4"><span className="text-sm font-bold text-muted">Tahanan terpanjang</span><strong className="mt-2 block text-3xl">{(metrics.averageContinuousHoldMs / 1000).toFixed(1)} dtk</strong></div></div></div><div className="grid gap-7">{bars.map((item) => <div key={item.label}><div className="flex items-end justify-between gap-4"><span className="font-black">{item.label}</span><strong className="text-2xl">{item.display}</strong></div><div className="mt-3 h-7 overflow-hidden rounded-full bg-divider"><m.div animate={{ width: `${(item.value / 120) * 100}%` }} className="h-full rounded-full" initial={{ width: reduceMotion ? `${(item.value / 120) * 100}%` : 0 }} style={{ backgroundColor: game.color }} transition={{ duration: 0.85, ease: 'easeOut' }} /></div></div>)}</div></div>;
  }
      ...
      <main className="relative mx-auto w-full max-w-[72rem] px-4 py-8 outline-none sm:px-6 lg:px-8 lg:py-12" tabIndex={-1}>
        ...
        {participant.isPending ? <ProfileSkeleton /> : participant.isError || !participant.data ? <div className="mt-8 grid min-h-56 place-items-center text-muted" role="status">Profil peserta tidak tersedia.</div> : (
          <>
            ...
            {participant.data.aggregateSummary && <section aria-labelledby="aggregate-summary-title" className="mt-10"><h2 className="m-0 text-3xl font-black" id="aggregate-summary-title">Ringkasan keseluruhan</h2><p className="mt-2 mb-0 text-lg text-muted">Diperbarui dari {participant.data.aggregateSummary.savedSessionsTotal} permainan tersimpan.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><article className="rounded-md border-2 border-divider bg-white p-5"><p className="m-0 text-xs font-black tracking-[0.08em] text-accent uppercase">Untuk peserta</p><p className="mt-3 mb-0 leading-7 font-bold">{participant.data.aggregateSummary.participantSummary}</p></article><article className="rounded-md border-2 border-divider bg-white p-5"><p className="m-0 text-xs font-black tracking-[0.08em] text-accent uppercase">Untuk dokter</p><p className="mt-3 mb-0 leading-7 font-bold">{participant.data.aggregateSummary.clinicianSummary}</p></article></div></section>}
