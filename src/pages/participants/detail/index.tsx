...
function OverallModePanel({ summary }: { summary: ParticipantDetailDto['modeSummaries'][number] | undefined }) {
  ...
  if (metrics.mode === 'MOTOR_GRIP') {
    ...
    return <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-center"><div><p className="m-0 text-sm font-black tracking-[0.08em] text-muted uppercase">Overall {summary.savedSessionsTotal} permainan</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="border-l-4 border-divider py-1 pl-4"><span className="text-sm font-bold text-muted">Rata-rata skor</span><strong className="mt-2 block text-3xl">{metrics.averageScore}</strong></div><div className="border-l-4 border-divider py-1 pl-4"><span className="text-sm font-bold text-muted">Tahanan terpanjang</span><strong className="mt-2 block text-3xl">{(metrics.averageContinuousHoldMs / 1000).toFixed(1)} dtk</strong></div></div></div><div className="grid gap-7">{bars.map((item) => <div key={item.label}><div className="flex items-end justify-between gap-4"><span className="font-black">{item.label}</span><strong className="text-2xl">{item.display}</strong></div><div className="mt-3 h-7 overflow-hidden rounded-full bg-divider"><m.div animate={{ width: `${(item.value / 5) * 100}%` }} className="h-full rounded-full" initial={{ width: reduceMotion ? `${(item.value / 5) * 100}%` : 0 }} style={{ backgroundColor: game.color }} transition={{ duration: 0.85, ease: 'easeOut' }} /></div></div>)}</div></div>;
  }
