...
import { ArrowLeft, Camera, Clock3, ImageOff, Pencil } from 'lucide-react';
...
export function ParticipantDetailPage() {
  ...
  async function saveParticipant(event: FormEvent<HTMLFormElement>) {
    ...
    setEditError('');
    try {
      await updateParticipant.mutateAsync({ displayName: normalized, image });
      setEditOpen(false);
    } catch (error) {
      ...
      {editOpen && (
        <div aria-labelledby="edit-participant-title" aria-modal="true" className="fixed inset-0 z-[90] grid place-items-center bg-ink/65 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditOpen(false); }} role="dialog">
          <form className="max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-lg bg-white p-6 text-ink shadow-[0_8px_0_rgba(23,23,17,0.28)] sm:p-8" noValidate onSubmit={saveParticipant}>
            <div className="flex items-center gap-4"><span className="grid size-14 place-items-center rounded-full bg-brand-soft"><Pencil aria-hidden className="size-7" /></span><div><h2 className="m-0 text-2xl font-black" id="edit-participant-title">Edit peserta</h2><p className="mt-1 mb-0 text-muted">Perbarui nama yang tampil.</p></div></div>
            <div className="mt-7"><Field autoFocus error={editError} label="Nama peserta" maxLength={100} name="participantDisplayName" onChange={(event) => { setName(event.target.value); setEditError(''); }} required value={name} /></div>
            <div className="mt-7 grid gap-3 sm:grid-cols-2"><Button disabled={updateParticipant.isPending} onClick={() => setEditOpen(false)} variant="secondary">Batal</Button><Button disabled={updateParticipant.isPending} type="submit">{updateParticipant.isPending ? 'Menyimpan…' : 'Simpan nama'}</Button></div>
          </form>
        </div>
      )}
      {avatarOpen && (
