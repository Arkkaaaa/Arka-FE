...
import { ArrowLeft, Camera, Clock3, ImageOff } from 'lucide-react';
...
import { AccountHeader, Button, Field, buttonClassName } from '../../../components/index.ts';
...
function ScoreTrendChart({ history }: { history: HistoryPageDto['items'] }) {
  ...
  const colors = Object.fromEntries(GAME_MODES.map((mode) => [mode.mode, mode.color])) as Record<GameMode, string>;
  const series = GAME_MODES.map((mode) => {
    const data = history.filter((item) => item.mode === mode.mode && item.score !== null && item.completedAt).slice(0, 7).reverse();
    ...
    return { ...mode, data, points, line: points.map((point) => `${point.x},${point.y}`).join(' '), average: data.length ? Math.round(data.reduce((sum, item) => sum + (item.score ?? 0), 0) / data.length) : null };
  });
  if (series.every((item) => item.data.length === 0)) return <div className="grid min-h-64 place-items-center text-center text-muted"><div><p className="m-0 text-lg font-black">Belum ada tren skor</p><p className="mt-2 mb-0">Grafik akan tampil setelah permainan tersimpan.</p></div></div>;
  return (
    <div>
      <div className="overflow-x-auto">
        <svg aria-label="Tren skor terbaru untuk tiga mode permainan" className="h-auto min-w-[34rem] w-full" role="img" viewBox={`0 0 ${width} ${height}`}>
          ...
          {series.map((item) => <g key={item.mode}>{item.points.length > 1 && <m.polyline animate={{ pathLength: 1, opacity: 1 }} fill="none" initial={reduceMotion ? { pathLength: 1, opacity: 1 } : { pathLength: 0, opacity: 0 }} points={item.line} stroke={colors[item.mode]} strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" transition={{ duration: 0.8, ease: 'easeOut' }} />}{item.points.map((point, index) => <m.g animate={{ opacity: 1, scale: 1 }} initial={reduceMotion ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.4 }} key={point.sessionId} style={{ transformBox: 'fill-box', transformOrigin: 'center' }} transition={{ delay: reduceMotion ? 0 : 0.12 * index, duration: 0.3 }}><circle cx={point.x} cy={point.y} fill="white" r="6" stroke={colors[item.mode]} strokeWidth="4" /><text fill="#171711" fontSize="12" fontWeight="800" textAnchor="middle" x={point.x} y={point.y - 13}>{point.score}</text></m.g>)}</g>)}
        </svg>
      </div>
      <div className="mt-2 grid gap-3 sm:grid-cols-3">{series.map((item) => <div className="rounded-sm bg-canvas/70 p-3" key={item.mode}><span className="flex items-center gap-2 font-black"><span aria-hidden className="size-3 rounded-full" style={{ backgroundColor: colors[item.mode] }} /><img alt="" aria-hidden className="size-6" src={item.emoji} />{item.title}</span><span className="mt-2 block text-sm font-bold text-muted">{item.average === null ? 'Belum ada data' : `Rata-rata ${item.average} · ${item.data.length} permainan`}</span></div>)}</div>
    </div>
  );
}
...
function OverallModePanel({ summary }: { summary: ParticipantDetailDto['modeSummaries'][number] | undefined }) {
  ...
  if (metrics.mode === 'MOTOR_GRIP') {
    const bars = [{ label: 'Kekuatan genggaman', value: metrics.averagePeakGripPercent, display: `${Math.round(metrics.averagePeakGripPercent)}%` }, { label: 'Target latihan tercapai', value: metrics.targetCompletionPercent, display: `${Math.round(metrics.targetCompletionPercent)}%` }];
    return <div className="grid gap-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-center"><div><p className="m-0 text-sm font-black tracking-[0.08em] text-muted uppercase">Overall {summary.savedSessionsTotal} permainan</p><div className="mt-5 grid grid-cols-2 gap-3"><div className="border-l-4 border-divider py-1 pl-4"><span className="text-sm font-bold text-muted">Rata-rata skor</span><strong className="mt-2 block text-3xl">{metrics.averageScore}</strong></div><div className="border-l-4 border-divider py-1 pl-4"><span className="text-sm font-bold text-muted">Durasi tahan</span><strong className="mt-2 block text-3xl">{(metrics.averageContinuousHoldMs / 1000).toFixed(1)} dtk</strong></div></div></div><div className="grid gap-7">{bars.map((item) => <div key={item.label}><div className="flex items-end justify-between gap-4"><span className="font-black">{item.label}</span><strong className="text-2xl">{item.display}</strong></div><div className="mt-3 h-7 overflow-hidden rounded-full bg-divider"><m.div animate={{ width: `${item.value}%` }} className="h-full rounded-full" initial={{ width: reduceMotion ? `${item.value}%` : 0 }} style={{ backgroundColor: game.color }} transition={{ duration: 0.85, ease: 'easeOut' }} /></div></div>)}</div></div>;
  }
  if (metrics.mode === 'GO_NO_GO') {
    const responses = [{ label: 'Tepat sasaran', value: metrics.totalHits, color: game.color }, { label: 'Tepat menahan', value: metrics.totalCorrectRejections, color: '#68a7e5' }, { label: 'Terlewat', value: metrics.totalMisses, color: '#e7b82c' }, { label: 'Respons keliru', value: metrics.totalFalsePositives, color: '#dc4c3f' }];
    let offset = 0;
      ...
      {editOpen && (
        <div aria-labelledby="edit-participant-title" aria-modal="true" className="fixed inset-0 z-[90] grid place-items-center bg-ink/65 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditOpen(false); }} role="dialog">
          <form className="max-h-[calc(100dvh-2rem)] w-full max-w-md overflow-y-auto rounded-lg bg-white p-6 text-ink shadow-[0_8px_0_rgba(23,23,17,0.28)] sm:p-8" noValidate onSubmit={saveParticipant}>
            ...
            <div className="mt-7 grid gap-5"><Field autoFocus error={editError} label="Nama peserta" maxLength={100} name="participantDisplayName" onChange={(event) => { setName(event.target.value); setEditError(''); }} required value={name} /><Field label="Tanggal lahir" max={new Date().toISOString().slice(0, 10)} name="participantDateOfBirth" onChange={(event) => setDateOfBirth(event.target.value)} type="date" value={dateOfBirth} /><fieldset className="m-0 border-0 p-0"><legend className="mb-2 font-black">Gender</legend><div className="grid grid-cols-2 gap-3"><label className={`flex min-h-12 cursor-pointer items-center justify-center rounded-sm border-2 font-black ${gender === 'MALE' ? 'border-ink bg-ink text-white' : 'border-divider'}`}><input checked={gender === 'MALE'} className="sr-only" name="participantGender" onChange={() => setGender('MALE')} type="radio" />Laki-laki</label><label className={`flex min-h-12 cursor-pointer items-center justify-center rounded-sm border-2 font-black ${gender === 'FEMALE' ? 'border-ink bg-ink text-white' : 'border-divider'}`}><input checked={gender === 'FEMALE'} className="sr-only" name="participantGender" onChange={() => setGender('FEMALE')} type="radio" />Perempuan</label></div></fieldset></div>
            <div className="mt-7 grid gap-3 sm:grid-cols-2"><Button disabled={updateParticipant.isPending} onClick={() => setEditOpen(false)} variant="secondary">Batal</Button><Button disabled={updateParticipant.isPending} type="submit">{updateParticipant.isPending ? 'Menyimpan…' : 'Simpan perubahan'}</Button></div>
          </form>
        </div>
      )}
      {avatarOpen && (
