...
function MotorGripChart({ metrics }: { metrics: Extract<GameMetrics, { mode: 'MOTOR_GRIP' }> }) {
  ...
  const points = metrics.gripSamples.map((sample) => ({ ...sample, x: left + ((sample.elapsedSecond - 1) / Math.max(durationSeconds - 1, 1)) * plotWidth, y: top + plotHeight - (sample.kilograms / 120) * plotHeight }));
  const line = points.map((point) => `${point.x},${point.y}`).join(' ');
  const targetY = top + plotHeight - (metrics.targetKilograms / 120) * plotHeight;
  return (
